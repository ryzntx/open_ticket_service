...
class ArtisanController extends Controller
{
    ...
    {
        if (app()->environment('production') && ! Gate::allows('artisan-gui')) {
            abort(403, 'Unauthorized.');
        }

        $commands = collect(Artisan::all())
            ->map(function ($command) {
                return $this->commandToArray($command);
            })
            ->filter()
            ->groupBy(function ($command) {
                return Str::of($command['name'])->before(':')->__toString();
            })
            ->toArray();

        // dd($commands);

        return view('admins.artisan.index', compact('commands'));
    }
    ...
    public function run(Request $request)
    {
        ...
        return back()
            ->with($res);
        // return redirect()->back()->withErrors(['command' => 'Command cannot be empty']);
    }
